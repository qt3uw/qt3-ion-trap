...
class FigureParameterConfig:
    def __init__(self):
        ...
        self.graph_file_name = 'acquisition/8-18_Trial18_data.txt'        # File to plot height & micromotion vs. voltage graphs for
...
def plot_height_fit(include_gaps=True, figsize=(3.5, 3), sigma = [1], file_name = ''):
    """
    ...
    parameters = ['charge_to_mass', 'v_dc']
    bounds = [(-1.E-2, -1.E-4)]
    ...
    y0_model_2 = trap.get_height_versus_dc_voltages(dc_voltages_2, include_gaps=include_gaps)
 
    
    method_2, = ax.plot(dc_voltages_2 - 40, y0_model_2 * 1.E3, color='k', label='Method 2')
    '''
   ...
   '''
    ...
    def merit_func(args):
        for i, key in enumerate(parameters):
            ...
            v_dc = trap.v_dc
        y0_model = trap.get_height_versus_dc_voltages(dc_voltages - v_dc, include_gaps=include_gaps)
    ...
    y0_meas = trap.get_height_versus_dc_voltages(dc_voltages_2, include_gaps=include_gaps)
    ...
    print(y0_meas)
    print(len(sigma))
    chi2_fit = chi2(y0, y0_meas[47:], sigma)
    chi2_extr = chi2(y0,y0_model_2[47:], sigma)
    ...
    method_1, = ax.plot(dc_voltages_2, y0_meas * 1.E3, color='k', linestyle='--', label='Method 1')
...
if __name__ == "__main__":
    sigma_T18 = [0.772, 0.490, 0.700, 0.4, 1.062, 1.024, 0.499, 0.894, 0.618, 0.748, 0.499, 1.143, 0.884, 0.596, \
             0.712, 0.957, 0.618, 0.442, 0.772, 0.499, 0.618, 0.699, 0.957, 0.806, 0.680, 0.249, 0, 0.680, \
             0.854, 1.062, 1.108, 1.147, 0.957, 0.618, 0.499, 0.966, 1.2, 0.573, 1.062, 0.596, 1.011, 1.720, \
             1.784, 0.929, 0.452]
    config = FigureParameterConfig()
    config.graph_file_name = 'acquisition/.txt'
    y_cuts_panel()
    e_field_panel()
    potential_energy_panel()
    plot_escape(figsize=(3.5, 3))
    plot_height_fit(figsize=(2.5, 3), include_gaps=True, sigma= sigma_T18, file_name = '8-18_Trial18_data')
