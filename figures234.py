...
class FigureParameterConfig:
    ...
    def __init__(self, uncert, height_file_name = "data/raw_micromotion/second_round_data_collection/Clean Data/02-28-2025_Trial" + TRIAL + "_data.txt", save_path = ["figures/figure_" + str(i) + "/" for i in range(2, 5)]):
        self.save_fig = True                                                   
        self.unc = uncert
        self.pixel_to_mm = uncert.pxl_to_mm                                     
        self.graph_file_name = height_file_name    
        self.hist_folder_name = "data/analyzed_micromotion/second_round_data_collection/"                     
...
def get_default_config(height_file_name = "data/raw_micromotion/second_round_data_collection/Clean Data/02-28-2025_Trial" + str(11) + "_data.txt", save_path = ["figures/figure_" + str(i) + "/" for i in range(2, 5)], \
                       uncert = Uncertainties(r_c = [np.nan, 16.053-0.178], N_c = [np.nan, 900-10])):
    """
    Returns a FigureParameterConfig object with specified uncert, height_file_name, save_path, uncert
        :param height_file_name: (str) The file name with which height data will be used in graphs
        :param save_path: (List[str]) A list of path names to save the planar trap paper figures
        :param uncert (Uncertainties()): An Uncertainties object with specified arguments r_c = [np.nan, 16.053-0.178], N_c = [np.nan, 900-10])
        :returns: FigureParameterConfig(uncert = uncert, height_file_name = height_file_name, save_path = save_path)
    """
...
def get_data(config, filename = None):
    ...
    Reads and sorts experimental data from a text file.
        :param config: (FigureParameterConfig) FigureParameterConfig object with data relevant to the plots
        :param filename: (str) File to extract data from. Only necessary for folder iteration in which the file is not graph_file_name
        :return: if filename == None: 
                    return Minimum Micromotion in [mm], charge-to-mass [Coulombs/kilograms], Votlage which pushes particle to the RF null, the experimental RF null height above the planae 
                 else:
                    return c2m 
    """
    data_list = []
...
def plot_height_fit(config, include_gaps=True, figsize=(3.5, 3)):
    ...
    Plots and saves experimental ion height as a function of applied voltage in addition to the predicted ion height
        ...
        :param config: (FigureParameterConfig) FigureParameterConfig object with data relevant to the plots
        :param include_gaps: (bool) Includes or excludes spatial gap between electrodes when calculating relevant fields.
        :param figsize: Tuple(float, float) Figure dimensions in inches
    ...
    ax.fill_between(-dc_voltages, y0_model_upper * 1.E3, y0_model_lower*1.E3, alpha = 0.3, color = COLORS["error"], hatch='oo')
...
if __name__ == "__main__":
    """
    Produces and saves plots of micromotion data.
    """
    U = Uncertainties(r_c = [np.nan, (16.053-0.178)*1e-3], N_c = [np.nan, 900-10], delta_r_c = [np.nan, (16.053-0.178) * 0.005 * 1e-3], delta_N_c = [np.nan, 1/(2*np.sqrt(12))], delta_cent = -1)
    for i in [19]:
